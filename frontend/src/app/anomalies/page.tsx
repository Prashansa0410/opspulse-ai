  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111827]/90">
          <span className="text-xs font-mono text-slate-600 dark:text-slate-400">Active Anomalies</span>
          <div className="text-2xl font-bold text-slate-900 dark:text-white font-mono mt-1">
            {anomalies.length}
          </div>
          <span className="text-[11px] text-slate-600 dark:text-slate-400">Evaluated against 7-day rolling baselines</span>
        </div>

        <div className="p-4 rounded-xl border border-rose-200 dark:border-rose-900/60 bg-rose-950/20">
          <span className="text-xs font-mono text-rose-700 dark:text-rose-300">Critical Incidents</span>
          <div className="text-2xl font-bold text-rose-600 dark:text-rose-400 font-mono mt-1">
            {anomalies.filter((a) => a.severity === "CRITICAL").length}
          </div>
          <span className="text-[11px] text-rose-700 dark:text-rose-300">Z-Score ≥ 2.5 sigma deviation</span>
        </div>

        <div className="p-4 rounded-xl border border-amber-200 dark:border-amber-900/60 bg-amber-950/20">
          <span className="text-xs font-mono text-amber-700 dark:text-amber-300">Warning Level</span>
          ...
          <span className="text-[11px] text-amber-700 dark:text-amber-300">Z-Score 1.8 - 2.4 sigma deviation</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="p-8 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111827] text-center text-slate-600 dark:text-slate-400">
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-sm font-bold text-slate-900 dark:text-white font-mono">{anom.metric_name}</span>
                    <span className="text-xs text-slate-600 dark:text-slate-400 font-mono">({anom.entity_name})</span>
                  </div>

                  <div className="flex items-center gap-3 text-xs font-mono">
                    <span className="text-slate-600 dark:text-slate-400">Z-Score: <strong className="text-slate-900 dark:text-white font-bold">{anom.z_score}σ</strong></span>
                    <span className="text-slate-600 dark:text-slate-400">Deviation: <strong className="text-rose-600 dark:text-rose-400">+{anom.deviation_percent}%</strong></span>
                  </div>
                </div>
                ...
                <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800/90 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                  ...
                  <div className="flex items-center gap-4 text-[11px] font-mono text-slate-600 dark:text-slate-400 shrink-0">
                    <span>Expected: {anom.expected_value}</span>
                    <span>Actual: <strong className="text-rose-600 dark:text-rose-400">{anom.actual_value}</strong></span>
                  </div>
                </div>
              </div>
            ))}
