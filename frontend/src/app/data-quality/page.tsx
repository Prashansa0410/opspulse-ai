      ...
      {report && (
        <div className="p-6 rounded-2xl border border-emerald-800/80 bg-emerald-950/20 grid grid-cols-1 sm:grid-cols-4 gap-4 items-center shadow-lg">
          <div className="sm:col-span-2 space-y-1">
            ...
            <div className="text-4xl font-extrabold text-neutral-900 dark:text-white font-mono">
              {report.dq_score_pct}%
            </div>
          ...
          <div className="p-3 rounded-xl bg-neutral-50 dark:bg-slate-900/90 border border-neutral-200 dark:border-slate-800 text-xs font-mono">
            <span className="text-neutral-600 dark:text-slate-400">Rules Passed</span>
          ...
          <div className="p-3 rounded-xl bg-neutral-50 dark:bg-slate-900/90 border border-neutral-200 dark:border-slate-800 text-xs font-mono">
            <span className="text-neutral-600 dark:text-slate-400">Critical / Warnings</span>
            <div className="text-xl font-bold text-neutral-800 dark:text-slate-200 mt-1">
              <span className="text-rose-400">{report.critical_issues_count}</span> / <span className="text-amber-400">{report.warnings_count}</span>
            </div>
          </div>
        </div>
      )}
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {report.rules_results.map((rule) => (
              <div
                key={rule.rule_id}
                className="p-4 rounded-xl border border-neutral-200 dark:border-slate-800 bg-white dark:bg-[#111827]/90 space-y-2 hover:border-neutral-300 dark:border-slate-700 transition-colors"
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-2.5">
                    <span className="font-mono text-xs text-blue-400 font-bold">{rule.rule_id}</span>
                    <span className="text-sm font-bold text-neutral-900 dark:text-white">{rule.rule_name}</span>
                ...
                <p className="text-xs text-neutral-700 dark:text-slate-300">{rule.description}</p>

                <div className="flex flex-wrap items-center gap-4 text-[11px] font-mono text-neutral-600 dark:text-slate-400 pt-2 border-t border-slate-800/80">
                  <span>Target Table: <strong className="text-neutral-800 dark:text-slate-200">{rule.table_name}</strong></span>
                  <span>Records Audited: <strong className="text-neutral-800 dark:text-slate-200">{rule.records_checked.toLocaleString()}</strong></span>
                  <span>Failures: <strong className={rule.failed_records_count > 0 ? "text-rose-400 font-bold" : "text-emerald-400"}>{rule.failed_records_count}</strong> ({rule.failure_rate_pct}%)</span>
                </div>
              </div>
            ))}
          </div>
