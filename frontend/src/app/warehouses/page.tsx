              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-base font-bold text-white font-mono">{wh.code}</span>
                      <span
                        className={
                          isCritical
                            ? "badge-critical"
                    ...
                    <div className="text-xs text-slate-300 font-medium mt-0.5">{wh.warehouse_name}</div>
                    <div className="text-[11px] text-slate-400 font-mono">{wh.city}, {wh.state}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs font-mono pt-2 border-t border-slate-800">
                  <div className="p-2 rounded bg-slate-900/80 border border-slate-800">
                    <span className="text-slate-400 text-[10px]">Avg Packing Time</span>
                    <div className="text-sm font-bold text-slate-200 mt-0.5">{wh.avg_packing_hours} hrs</div>
                  </div>
                  <div className="p-2 rounded bg-slate-900/80 border border-slate-800">
                    <span className="text-slate-400 text-[10px]">SLA Breach Rate</span>
                    <div className={`text-sm font-bold mt-0.5 ${wh.avg_sla_breach_rate > 10 ? "text-rose-400" : "text-slate-200"}`}>
                      {wh.avg_sla_breach_rate}%
                    </div>
                  </div>
                  <div className="p-2 rounded bg-slate-900/80 border border-slate-800">
                    <span className="text-slate-400 text-[10px]">Assigned Orders</span>
                    <div className="text-sm font-bold text-slate-200 mt-0.5">{wh.total_orders_assigned.toLocaleString()}</div>
                  </div>
                  <div className="p-2 rounded bg-slate-900/80 border border-slate-800">
                    <span className="text-slate-400 text-[10px]">Active Backlog</span>
                ...
                {isCritical && (
                  <div className="p-2.5 rounded bg-rose-950/40 border border-rose-900/70 text-[11px] text-rose-300 flex items-center justify-between">
                    ...
                    <Link href="/simulations" className="underline font-mono font-bold text-white hover:text-rose-200">
                      Reallocate
                    </Link>
                  </div>
                )}
