...
export const ExplainModal: React.FC<ExplainModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-fadeIn">
      <div className="bg-[#0f172a] border border-slate-700/80 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 space-y-6">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-white font-mono">
                Explainability Breakdown: <span className="text-rose-400">On-Time Delivery Decline</span>
              </h2>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800">
                <span className="text-xs text-slate-400">Baseline OTD</span>
              ...
              <div className="p-3.5 rounded-xl bg-rose-950/20 border border-rose-900/70">
              ...
              <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800">
                <span className="text-xs text-slate-400">GMV At Risk</span>
            ...
            <div className="p-4 rounded-xl bg-blue-950/30 border border-blue-800/60 space-y-1">
            ...
            <div className="space-y-2.5">
              ...
              <div className="space-y-2">
                {data.stage_breakdown.map((st, idx) => (
                  <div key={idx} className="p-3 rounded-lg bg-slate-900/70 border border-slate-800 flex items-center justify-between text-xs">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5">
                  {data.top_affected_warehouses.slice(0, 3).map((w) => (
                    <div key={w.warehouse_id} className="p-2.5 rounded-lg bg-slate-900/80 border border-slate-800 text-xs flex items-center justify-between">
                      <div>
                        <div className="font-medium text-slate-200">{w.code} ({w.warehouse_name})</div>
                        <div className="text-[11px] text-slate-400">{w.breach_count} breaches ({w.breach_rate}% breach rate)</div>
                      </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5">
                  {data.top_affected_carriers.slice(0, 3).map((c) => (
                    <div key={c.carrier_id} className="p-2.5 rounded-lg bg-slate-900/80 border border-slate-800 text-xs flex items-center justify-between">
                      <div>
                        <div className="font-medium text-slate-200">{c.carrier_name}</div>
                        <div className="text-[11px] text-slate-400">{c.delayed_shipments} delayed shipments ({c.delay_rate}% delay rate)</div>
                      </div>
